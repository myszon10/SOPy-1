...
char* circular_buffer_dequeue(circular_buffer *bufferArgs) 
{
    ...
    while(bufferArgs->count == 0) // bufor jest pusty
    {
        // ERR("Cannot remove from an empty buffer");
        usleep(5000); // wait 5ms (busy waiting)
    }

    pthread_mutex_lock(&bufferArgs->mxBuffer);
    char *item = bufferArgs->buffer[bufferArgs->tail];

    bufferArgs->tail = (bufferArgs->tail + 1) % BUFFER_SIZE; // przesunięcie wskaźnika tail
