...
typedef struct argsModify
{
    ...
    int year; // bierzący rok studenta
} argsModify_t;
...
{
    // wątek będzie anulowany tylko w punktach anulowania (w bezpiecznym momencie)
...
void msleep(UINT milisec)
{
    time_t sec = (int)(milisec / 1000); // liczba pełnych sekund
    milisec = milisec - (sec * 1000); // liczba pozostałych nanosekund
    timespec_t req = {0};
    req.tv_sec = sec;
...
{
    int idx;
    if (0 == studentsList->present) // czy są jeszcze studenci
        return;
    do
    {
